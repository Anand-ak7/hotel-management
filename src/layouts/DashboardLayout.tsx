...
function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <aside className="flex h-full w-72 flex-col overflow-hidden bg-royal-ink text-white shadow-royal">
      <div className="bg-gradient-to-br from-royal-navy via-royal-ink to-slate-950 p-5">
        <div className="mb-7 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-extrabold tracking-wide">Hotel Desk</p>
            </div>
          </div>
          {onClose && (
            <button
      ...
      <nav className="flex flex-1 flex-col gap-1 bg-royal-ink px-4 py-5">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
      ...
      <div className="m-4 rounded-xl border border-white/10 bg-white/10 p-3 backdrop-blur">
  ...
  return (
    <div className="min-h-screen bg-transparent">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-20 border-b border-app-border bg-white/85 px-4 shadow-sm backdrop-blur-xl lg:px-8">
        ...
        <main className="mx-auto max-w-7xl p-4 sm:p-5 lg:p-8">
          <Outlet />
        </main>
      </div>
    </div>
